...
const fixSolutionsInput = z.array(z.object({ title: z.string().trim().min(2).max(400), explanation: z.string().trim().max(12000).nullable().optional(), command: z.string().trim().max(12000).nullable().optional(), warning: z.string().trim().max(12000).nullable().optional(), kind: z.enum(["inspect", "change", "verify", "recover"]).default("inspect"), risk: z.enum(["read_only", "reversible", "system_change"]).default("read_only"), verification: z.string().trim().max(12000).nullable().optional(), rollback: z.string().trim().max(12000).nullable().optional(), sourceUrl: z.string().trim().url().max(2048).nullable().optional() })).min(1).max(80);
