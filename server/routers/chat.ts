...
import { z } from "zod";
import { chatMessages, chatSessions, linuxFixes, setupGuides, wikiArticles } from "../../drizzle/schema";
...
export const chatRouter = router({
  contextForLocal: activeUserProcedure.input(z.object({ question: z.string().trim().min(2).max(2500) })).mutation(async ({ input }) => retrieveContext(input.question)),
