...
export const desktopRouter = router({
  ...
  games: router({
    list: publicProcedure.input(pageInput.extend({ q: z.string().optional(), distributionId: z.number().optional(), gpuId: z.number().optional(), cpuId: z.number().optional(), compatibility: z.string().optional(), platform: z.string().optional(), tagSlugs: z.array(z.string()).optional(), sort: z.string().optional() })).query(({ input }) => gameList(input)),
  ...
  user: router({
    dashboard: publicProcedure.query(() => ({ user: localUser, profiles: store().all<any>("SELECT id, name, cpu_id AS cpuId, gpu_id AS gpuId, ram_id AS ramId, distribution_id AS distributionId, kernel_version AS kernelVersion, driver_version AS driverVersion, proton_version AS protonVersion, is_active AS isActive, updated_at AS updatedAt FROM profiles ORDER BY is_active DESC, updated_at DESC"), favorites: store().all<any>("SELECT g.id, g.slug, g.title, g.description AS shortDescription, g.steam_app_id AS steamAppId FROM favorites f JOIN games g ON g.id = f.game_id ORDER BY f.created_at DESC"), savedGuides: store().all<any>("SELECT g.id, g.slug, g.title, g.description, g.difficulty, g.provenance FROM saved_guides s JOIN setup_guides g ON g.id = s.guide_id ORDER BY s.created_at DESC") })),
    profiles: router({ list: publicProcedure.query(() => store().all<any>("SELECT id, name, cpu_id AS cpuId, gpu_id AS gpuId, ram_id AS ramId, distribution_id AS distributionId, kernel_version AS kernelVersion, driver_version AS driverVersion, proton_version AS protonVersion, is_active AS isActive, updated_at AS updatedAt FROM profiles ORDER BY is_active DESC, updated_at DESC")), upsert: publicProcedure.input(z.any()).mutation(({ input }) => { if (input.isActive) store().run("UPDATE profiles SET is_active = 0"); if (input.id) { store().run("UPDATE profiles SET name = ?, cpu_id = ?, gpu_id = ?, kernel_version = ?, driver_version = ?, proton_version = ?, is_active = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", [input.name, input.cpuId || null, input.gpuId || null, input.kernelVersion || null, input.driverVersion || null, input.protonVersion || null, input.isActive ? 1 : 0, input.id]); return { id: input.id }; } const result = store().run("INSERT INTO profiles (name, cpu_id, gpu_id, kernel_version, driver_version, proton_version, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)", [input.name, input.cpuId || null, input.gpuId || null, input.kernelVersion || null, input.driverVersion || null, input.protonVersion || null, input.isActive ? 1 : 0]); return { id: Number(result.lastInsertRowid) }; }), remove: publicProcedure.input(z.object({ id: z.number() })).mutation(({ input }) => { store().run("DELETE FROM profiles WHERE id = ?", [input.id]); return { success: true }; }) }),
