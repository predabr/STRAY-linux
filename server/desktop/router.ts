...
function desktopAiAnswer(question: string) {
  ...
  return {
    answer: `### Leitura do caso\n${known}${memories.length ? `\nMemória local consentida: ${memories.map((item) => item.summary).join(" · ")}.` : ""}\n\n### Evidência disponível\n${sourceText}\n\n### Lacunas e pré-requisitos\nO modo desktop não possui benchmark, telemetria de FPS, histórico de performance ou compatibilidade por ambiente para esta pergunta. Antes de seguir uma fonte, confirme a distribuição, versão, runtime e sintoma observados no seu relatório local.\n\n### Ações seguras\n${citations.length ? "Leia as fontes listadas e execute o Scanner novamente após qualquer alteração para verificar apenas campos detectáveis." : "Abra o Scanner, gere um relatório local e consulte LinuxFix ou guias por tema antes de mudar configurações."}\n\n### Risco e reversão\nNenhuma alteração é executada pelo Stray AI. Quando uma fonte não documentar reversão, não trate a ação como reversível.\n\n### Limites\nA resposta usa somente o perfil local, memória com consentimento e conteúdo empacotado no SQLite. Ela não presume que uma solução funcione no seu ambiente, nem conclui FPS, compatibilidade, causa raiz ou resultado sem evidência específica.`,
    citations,
