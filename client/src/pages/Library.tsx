...
import { gameArtworkCandidates } from "@/lib/gameArtwork";
...
export default function Library() {
  ...
  const error = notice?.tone === "error" ? notice.message : null;
  ...
  const scan = useCallback(async () => {
    ...
    try { setGames((await desktop.library.scan()).games); setLastLocalRead(Date.now()); setNotice({ tone: "success", message: "Leitura local atualizada. Nenhum dado foi enviado." }); }
    catch (cause) { setNotice({ tone: "error", message: cause instanceof Error ? cause.message : "Não foi possível ler Steam e Heroic locais. Nenhum dado foi enviado." }); }
  ...
  async function launch(gameId: string) {
    if (!desktop?.library) return;
    setLaunching(gameId); setNotice(null);
    try { await desktop.library.launch(gameId); setNotice({ tone: "success", message: "A solicitação foi enviada ao Steam. O launcher decide se o jogo abre." }); }
    catch (cause) { setNotice({ tone: "error", message: cause instanceof Error ? cause.message : "Não foi possível solicitar a execução pelo Steam." }); }
    finally { setLaunching(null); }
  }

  async function reveal(gameId: string) {
    if (!desktop?.library) return;
    setRevealing(gameId); setNotice(null);
    try { await desktop.library.reveal(gameId); setNotice({ tone: "success", message: "A pasta de instalação foi aberta localmente." }); }
    catch (cause) { setNotice({ tone: "error", message: cause instanceof Error ? cause.message : "Não foi possível abrir a pasta de instalação." }); }
  ...
  async function addExternal() {
    ...
    try {
      ...
      setNotice({ tone: "success", message: "A pasta foi adicionada à leitura local." });
    } catch (cause) { setNotice({ tone: "error", message: cause instanceof Error ? cause.message : "Não foi possível ler a pasta selecionada." }); }
  }
...
function InstalledGameCard({ game, launching, revealing, onLaunch, onReveal }: { game: EnrichedGame; launching: boolean; revealing: boolean; onLaunch: () => void; onReveal: () => void }) {
  ...
  const artwork = gameArtworkCandidates(game.coverUrl || game.catalog?.coverImageUrl, game.appId ?? game.catalog?.steamAppId);
  const cover = artwork[artworkIndex] ?? null;
	  return <Card className="stray-library-card group overflow-hidden">{cover ? <div className="relative aspect-[3/4] overflow-hidden bg-black/70"><img src={cover} alt={`Capa de ${game.name}`} className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-[1.025]" loading="lazy" onError={() => setArtworkIndex((current) => current + 1)} /><span className="absolute left-3 top-3 rounded-md border border-white/10 bg-black/50 px-2 py-1 font-tech text-[9px] tracking-[.12em] text-white/80 backdrop-blur">{artworkIndex > 0 ? "ARTE ALTERNATIVA" : game.coverSource === "heroic-local-metadata" ? "HEROIC LOCAL" : "ARTE DO CATÁLOGO"}</span></div> : <div className={`relative flex aspect-[3/4] flex-col justify-between overflow-hidden bg-gradient-to-br ${fallbackClass} p-4`}><span className="relative self-start rounded-md border border-white/10 bg-black/30 px-2 py-1 font-tech text-[9px] tracking-[.12em] text-white/80">ARTE AUSENTE</span><div className="relative"><p className="font-tech text-[10px] uppercase tracking-[.16em] text-white/65">{game.launcher === "heroic" ? "Heroic local" : game.launcher === "external" ? "Pasta externa" : "Steam local"}</p><p className="mt-2 max-w-[18rem] text-xs leading-5 text-white/70">Sem arte disponível nesta leitura.</p></div></div>}<CardHeader className="pb-3"><div className="flex items-start justify-between gap-2"><CardTitle className="line-clamp-1 text-base">{game.name}</CardTitle><Badge variant="outline" className="shrink-0 text-[10px]">{game.installationType === "flatpak" ? "Flatpak" : game.installationType === "external" ? "Manual" : "Nativa"}</Badge></div><CardDescription className="line-clamp-2">{game.catalog?.shortDescription || (game.launcher === "steam" ? `Steam App ID ${game.appId}` : game.launcher === "heroic" ? "Jogo detectado pelo Heroic" : "Pasta adicionada manualmente")}</CardDescription></CardHeader><CardContent className="space-y-2"><p className="line-clamp-1 text-xs text-muted-foreground">{game.installDir ? `Instalação: ${game.installDir}` : "Pasta de instalação não informada"}</p>{game.launcher === "steam" ? <Button className="w-full" size="sm" onClick={onLaunch} disabled={launching}><Play className="mr-2 h-4 w-4" />{launching ? "Abrindo…" : "Jogar pelo Steam"}</Button> : null}<Button className="w-full" size="sm" variant="outline" onClick={onReveal} disabled={revealing}><FolderOpen className="mr-2 h-4 w-4" />{revealing ? "Abrindo pasta…" : "Abrir instalação"}</Button>{game.catalog?.slug ? <Button asChild className="w-full" size="sm" variant="ghost"><Link href={`/games/${game.catalog.slug}`}><ExternalLink className="mr-2 h-4 w-4" />Ver detalhes no GameHub</Link></Button> : null}</CardContent></Card>;
}
