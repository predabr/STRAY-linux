...
type InstalledGame = { id: string; appId: number | null; externalId: string; name: string; installDir: string | null; libraryPath: string; installationType: "native" | "flatpak"; launcher: "steam" | "heroic"; store: "steam" | "epic"; coverUrl: string | null; coverSource: "heroic-local-metadata" | null };
...
export default function Library() {
  ...
  return <div className="min-h-screen bg-background"><SiteHeader /><main className="container py-8 md:py-12"><div className="mx-auto max-w-6xl space-y-6"><PageBreadcrumbs items={[{ label: "Stray Linux", href: "/" }, { label: "Minha biblioteca" }]} />
    <section className="flex flex-col justify-between gap-5 rounded-2xl border border-primary/20 bg-primary/[0.025] p-6 md:flex-row md:items-end"><div><Badge variant="outline" className="border-primary/30 bg-primary/5 text-primary">BIBLIOTECA LOCAL / STEAM + HEROIC</Badge><h1 className="mt-3 text-3xl font-semibold tracking-tight">Meus jogos instalados.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">O Stray Linux lê manifestos Steam e instalações Epic já registradas pelo Heroic sob sua solicitação. Não acessa conta, token, tempo jogado, nuvem ou configurações de execução.</p></div>{desktop?.library ? <Button onClick={scan} disabled={loading}><RefreshCw className={`mr-2 h-4 w-4 ${loading ? "animate-spin" : ""}`} />{loading ? "Lendo biblioteca…" : "Atualizar biblioteca"}</Button> : null}</section>
    {!desktop?.library ? <Card><CardContent className="flex flex-col items-center gap-3 p-10 text-center"><HardDrive className="h-8 w-8 text-primary" /><p className="font-medium">Disponível no aplicativo Stray Linux para desktop</p><p className="max-w-xl text-sm leading-6 text-muted-foreground">Abra esta página pelo executável Electron no Linux para detectar instalações locais da Steam e do Heroic sem enviar a biblioteca a nenhum serviço.</p></CardContent></Card> : <><section className="grid gap-4 md:grid-cols-[1fr_auto]"><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Filtrar jogos instalados" aria-label="Filtrar jogos instalados" /><Card className="flex items-center"><CardContent className="flex items-center gap-2 p-3 text-sm"><Gamepad2 className="h-4 w-4 text-primary" /><strong>{games.length}</strong> instalações detectadas</CardContent></Card></section>{error ? <p className="rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">{error}</p> : null}<section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{visibleGames.map((game) => <Card key={game.id} className="group overflow-hidden transition-transform duration-200 hover:-translate-y-0.5">{game.coverUrl ? <img src={game.coverUrl} alt={`Capa local de ${game.name}`} className="h-36 w-full object-cover" loading="lazy" /> : <div className="flex h-24 items-end bg-[radial-gradient(circle_at_20%_0%,hsl(var(--primary)/.28),transparent_42%),linear-gradient(125deg,hsl(var(--card)),hsl(var(--muted)/.28))] p-4"><span className="font-tech text-[10px] uppercase tracking-[.16em] text-white/50">{game.launcher === "heroic" ? "Heroic local" : "Steam local"}</span></div>}<CardHeader className="pb-3"><div className="flex items-start justify-between gap-2"><CardTitle className="line-clamp-1 text-base">{game.name}</CardTitle><Badge variant="outline" className="shrink-0 text-[10px]">{game.installationType === "flatpak" ? "Flatpak" : "Nativa"}</Badge></div><CardDescription>{game.launcher === "steam" ? `Steam App ID ${game.appId}` : "Epic detectado pelo Heroic"}{game.coverSource ? " · capa dos metadados locais" : ""}</CardDescription></CardHeader><CardContent><p className="line-clamp-1 text-xs text-muted-foreground">{game.installDir ? `Instalação: ${game.installDir}` : "Pasta de instalação não informada"}</p>{game.launcher === "steam" ? <Button className="mt-4 w-full" size="sm" onClick={() => launch(game.id)} disabled={launching === game.id}><Play className="mr-2 h-4 w-4" />{launching === game.id ? "Abrindo…" : "Jogar pelo Steam"}</Button> : <Button className="mt-4 w-full" size="sm" variant="outline" disabled><Play className="mr-2 h-4 w-4" />Abra pelo Heroic</Button>}</CardContent></Card>)}</section>{games.length === 0 && !loading ? <Card className="border-dashed"><CardContent className="flex flex-col items-center gap-3 p-10 text-center"><ShieldCheck className="h-8 w-8 text-primary" /><p className="font-medium">Nenhuma biblioteca lida ainda</p><p className="max-w-xl text-sm leading-6 text-muted-foreground">Use “Atualizar biblioteca” para ler os manifestos instalados. A leitura cobre Steam nativa/Flatpak e instalações Epic já registradas pelo Heroic, sem enviar informação à rede.</p></CardContent></Card> : null}</>}</div></main></div>;
}
