...
import { FolderPlus, Gamepad2, HardDrive, LibraryBig, Play, RefreshCw, ShieldCheck } from "lucide-react";
...
function InstalledGameCard({ game, launching, onLaunch }: { game: InstalledGame; launching: boolean; onLaunch: () => void }) {
  ...
  return <Card className="stray-surface group overflow-hidden transition-transform duration-200 hover:-translate-y-0.5">{game.coverUrl ? <img src={game.coverUrl} alt={`Capa local de ${game.name}`} className="h-36 w-full object-cover" loading="lazy" /> : <div className={`relative flex h-24 items-end overflow-hidden bg-gradient-to-br ${fallbackClass} p-4`}><div aria-hidden="true" className="absolute -right-7 -top-10 h-28 w-28 rounded-full border border-cyan-100/10" /><div aria-hidden="true" className="absolute -bottom-16 left-1/3 h-24 w-24 rounded-full border border-white/8" /><span className="relative font-tech text-[10px] uppercase tracking-[.16em] text-white/65">{game.launcher === "heroic" ? "Heroic local" : game.launcher === "external" ? "Pasta externa" : "Steam local"}</span></div>}<CardHeader className="pb-3"><div className="flex items-start justify-between gap-2"><CardTitle className="line-clamp-1 text-base">{game.name}</CardTitle><Badge variant="outline" className="shrink-0 text-[10px]">{game.installationType === "flatpak" ? "Flatpak" : game.installationType === "external" ? "Manual" : "Nativa"}</Badge></div><CardDescription>{game.launcher === "steam" ? `Steam App ID ${game.appId}` : game.launcher === "heroic" ? "Epic detectado pelo Heroic" : "Pasta adicionada manualmente · origem não inferida"}{game.coverSource ? " · capa dos metadados locais" : ""}</CardDescription></CardHeader><CardContent><p className="line-clamp-1 text-xs text-muted-foreground">{game.installDir ? `Instalação: ${game.installDir}` : "Pasta de instalação não informada"}</p>{game.launcher === "steam" ? <Button className="mt-4 w-full" size="sm" onClick={onLaunch} disabled={launching}><Play className="mr-2 h-4 w-4" />{launching ? "Abrindo…" : "Jogar pelo Steam"}</Button> : <Button className="mt-4 w-full" size="sm" variant="outline" disabled><Play className="mr-2 h-4 w-4" />{game.launcher === "heroic" ? "Abra pelo Heroic" : "Organização local"}</Button>}</CardContent></Card>;
}
