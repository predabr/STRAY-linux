...
function ContextCard({ loading, activeProfile, latestSnapshot, visitor }: { loading: boolean; activeProfile: { name: string; kernelVersion: string | null; driverVersion: string | null; protonVersion: string | null } | undefined; latestSnapshot?: { label: string; createdAt: string }; visitor: boolean }) { return <Card className="stray-surface shadow-none"><CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><MonitorCog className="h-4 w-4 text-cyan-200" />Contexto ativo</CardTitle><CardDescription>Campos conhecidos que podem orientar a resposta atual.</CardDescription></CardHeader><CardContent>{visitor ? <div className="stray-empty-state rounded-xl p-3"><p className="text-sm font-medium">Modo visitante</p><p className="mt-1 text-xs leading-5 text-muted-foreground">A resposta consulta apenas conteúdo publicado. Nenhum histórico ou perfil pessoal é salvo ou lido.</p></div> : loading ? <p className="text-sm text-muted-foreground">Carregando perfil…</p> : activeProfile ? <div className="space-y-2 text-xs"><ContextRow label="Perfil" value={activeProfile.name} /><ContextRow label="Kernel" value={activeProfile.kernelVersion || "não informado"} /><ContextRow label="Driver" value={activeProfile.driverVersion || "não informado"} />{latestSnapshot ? <ContextRow label="Scanner" value={new Date(latestSnapshot.createdAt).toLocaleString("pt-BR")} /> : null}<ContextRow label="Proton" value={activeProfile.protonVersion || "não informado"} /></div> : <div className="stray-empty-state rounded-xl p-3"><p className="text-sm font-medium">Nenhum perfil ativo</p><p className="mt-1 text-xs leading-5 text-muted-foreground">O Stray AI poderá declarar conteúdo publicado, mas não comparar com seu ambiente até existir um perfil.</p><Link href="/dashboard/pc" className="mt-3 inline-block text-xs font-medium text-cyan-200 hover:underline">Configurar Meu PC</Link></div>}</CardContent></Card>; }
