...
A apresentação pública utiliza uma composição editorial própria com navegação vertical, brilhos discretos com suporte a redução de movimento e downloads verificáveis. A release 1.1.4 corrige o bootstrap Linux: o launcher encaminha explicitamente o `sql-wasm.wasm` incluído em `resources`, captura os logs do servidor local e não depende do diretório de trabalho do menu do Arch.

Os caminhos públicos de download da série 1.1.4 respondem com redirecionamento HTTP para o artefato verificado da plataforma escolhida. Assim, futuras renovações de artefatos não quebram os links usados pela página, pelo README ou por instalações existentes.
