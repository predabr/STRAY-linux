...
A apresentação pública utiliza uma composição editorial própria com navegação vertical, brilhos discretos com suporte a redução de movimento e downloads verificáveis. A release 1.1.8 corrige o entrypoint empacotado: Vite e `vite.config.ts` agora existem apenas no entrypoint de desenvolvimento e não são incluídos no servidor executado pelo Pacman. O launcher também grava stdout, stderr e o caminho exato do log persistente antes de apresentar uma falha.

Os caminhos públicos de download da série 1.1.8 respondem com redirecionamento HTTP para o artefato verificado da plataforma escolhida. Assim, futuras renovações de artefatos não quebram os links usados pela página, pelo README ou por instalações existentes.
